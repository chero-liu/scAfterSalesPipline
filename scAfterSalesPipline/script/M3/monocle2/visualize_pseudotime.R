...
plot_genes_branched_pseudotime_one_gene <- function (cds, branch_states = NULL, branch_point = 1, branch_labels = NULL,
...
if (is.null(opt$vismethod)) {
    ...
    vismethods <- c("heatmap",  "treeplot", "ridgeplot", "module") #"trajectoryplot",,"expressplot_line","expressplot",
} else {
...
if (is.null(opt$one_gene)) {
    one_gene = FALSE
} else {
    one_gene = opt$one_gene
}
...
for (vismethod in vismethods) {
    ...
    if (vismethod == "expressplot" && one_gene == FALSE) {
    ...
    if (vismethod == "expressplot" && one_gene == TRUE) {
      for (i in seq(1, length(to_be_tested), 10)) {
            j <- min(i + 9, length(to_be_tested))
            to_be_tested_sub <- to_be_tested[i:j]
            cds_subset <- gbm_cds[to_be_tested_sub, ]
            ...
            if (is.null(branchpoint)) { # expressplot without branchtime
                for ( k in seq(1, length(to_be_tested), 1)){
                   gene <- to_be_tested_sub[k]
                   cds_subset_one = cds_subset[gene,]
                   g <- plot_genes_in_pseudotime(cds_subset_one, color_by = opt$groupby, cell_size = opt$pointsize, ncol = opt$numcol) + scale_colour_manual(values = new_celltype_pal) + guides(colour = guide_legend(override.aes = list(size = 1.5)))
                   ggsave(file.path(output_dir, paste0("pseudotime_gene_express_", gene, ".pdf")), plot=g, height = 6, width=7,bg="white")
                   ggsave(file.path(output_dir, paste0("pseudotime_gene_express_", gene, ".png")), plot=g, height = 6, width=7, dpi = 1000,bg="white")
              }
        }else { # expressplot with branchtime
                new_cds <- buildBranchCellDataSet(cds_subset, branch_point = branchpoint, progenitor_method = "duplicate")
                # cell_fate1
            ...
            branch_labels <- c(paste("State", paste(sort(cell_fate1_label), collapse = "-")), paste("State", paste(sort(cell_fate2_label), collapse = "-")))
                #branch_labels <- c(paste("State", paste(sort(setdiff(cell_fate1, cell_fate2)), collapse = "-")), paste("State", paste(sort(setdiff(cell_fate2, cell_fate1)), collapse = "-")))
                p <- plot_genes_branched_pseudotime_one_gene(cds_subset, color_by = opt$groupby, branch_point = branchpoint, cell_size = opt$pointsize, ncol = opt$numcol, branch_labels = branch_labels) 
                for ( k in seq(1, length(to_be_tested), 1)){
                    gene <- to_be_tested_sub[k]
                    df <- subset(p, feature_label==gene)
					df$Branch = as.factor(as.character(df$Branch))
                    df$feature_label = droplevels(df$feature_label)
                    color_by=opt$groupby
                    cell_size=opt$pointsize
                    ncol = opt$numcol
                    reducedModelFormulaStr = NULL
                    nrow = NULL
                    method = "fitting"
                    min_expr=NULL
                q <- ggplot(aes(Pseudotime, expression), data = df)
                    if (is.null(color_by) == FALSE) {
                    ...
                    g <- q+scale_colour_manual(values = new_celltype_pal) + guides(colour = guide_legend(override.aes = list(size = 1.5)))
                    g <- g +  ggplot2::theme(aspect.ratio = 1/1)
                
                    ggsave(file.path(output_dir, paste0("pseudotime_gene_express_branchtime_", gene, ".pdf")), plot=g, height = 6, width=7,bg="white")
                    ggsave(file.path(output_dir, paste0("pseudotime_gene_express_branchtime_", gene, ".png")), plot=g, height = 6, width=7, dpi = 1000,bg="white")
            } 
            }
        }
    }
