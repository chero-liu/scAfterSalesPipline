...
from scAfterSalesPipline.tools.M2.gsea import Gsea
...
class Module2(Step):
    ...
    def run(self):
        ...
        if self.Gsea in self.analysis_list:
            input = get_input(
                analysis=self.Gsea,
                type=self.data[self.param][self.type],
                module=self.data[self.module],
            ...
            for gmt in gmts:
                self.gsea(
                    input=input,
                    gmt=gmt,
                    contrast=self.data[self.param][self.Gsea][self.contrast],
                )
