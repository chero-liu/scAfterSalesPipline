    ...
    if module == "M1":
        if input == None:
            if analysis == "Clustering" and subobj != None:
                ...
                if useColname == None or useColname == "new_celltype":
                    return input_Manualanno
                else:
                    return input_Clustering
            elif analysis == "Diffexp" and subobj == None:
...
class ModuleFun:
    ...
    def __init__(self, data, input, analysis, type):
        ...
        try:
            ...
            sys.exit(
                f"Error: Please choose one of [human,mouse,rat,dog,mmul(猕猴),macaca_fascicularis(食蟹猴),sus_scrofa(猪),gallus_gallus(鸡),oar(绵羊),Capra_hircus(山羊),Caenorhabditis_elegans(秀丽隐杆线虫)], and the author(chenglong.liu@oebiotech.com) can be contacted to add {e}."
            )
