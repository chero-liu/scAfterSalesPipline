...
from scAfterSalesPipline.init.__init__ import M1DIRECTORIES
...
class Module1(Step):
    ...
    def init_yaml(self):
        ...
        with open(os.path.join(self.outdir, "config", "cfgM1.yaml"), "w") as file:
            yaml.dump(data, file)

    def load_data(self):
        type_path = os.path.join(self.outdir, "result", "M1", self.type)
