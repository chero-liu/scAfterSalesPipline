...
class Diffexp(ModuleFun):
    ...
    def init_param(self):
        ...
        if self.cluster_useColname == None:
            if self.subobj != None and self.subobj[0].isdigit():
                self.cluster_useColname = "clusters"
            else:
        ...
        if self.subobj != None:
            ...
            self.outdir = os.path.join(
                self.outdir,self.type, "SubDiff","_".join(self.subobj)
            )
            self.prefix = f"""{"_".join(self.subobj)}_of_{self.type}_{self.contrast.split(":")[0]}-vs-{self.contrast.split(":")[1]}_{self.prefix}"""
        else:
            self.predicate = ""
