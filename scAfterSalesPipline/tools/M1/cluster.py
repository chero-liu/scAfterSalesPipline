    ...
    def init_param(self):
        ...
        if self.subobj != None:
            ...
            self.outdir = os.path.join(
                self.outdir, "_".join(self.subobj)+f'_of_{self.type}', self.analysis
            )
        else:
            self.predicate = ""
            self.annolevel = "main"
...
Rscript  {self.script}/sctool \\
    ...
    --pointsize  0.5   \\
    --palette customecol2  \\
    --ref  {os.path.dirname(os.path.dirname(self.geneanno))}
...
"""
        if self.subobj != None:
            self.prefix = f"""{"_".join(self.subobj)}_of_{self.type}_{self.prefix}"""
        else:
