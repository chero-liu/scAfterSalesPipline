...
#    }
    ...
    if (total_char < 40){
        ...
        base_row_height = 0.5 
        calculated_height =  5 + base_row_height * nrows

        p = plot_cell_trajectory(gbm_cds, color_by = color_group, cell_size = opt$pointsize, show_branch_points = F) + scale_colour_manual( values = user_color_pal) + guides(colour = guide_legend(override.aes = list(size=2),nrow=nrows)) +theme(aspect.ratio = 1/1)
        ggsave(file.path(output_dir,paste0("cell_trajectory_color_by_",color_group,".pdf",collapse = "")), plot = p,bg="white",height= calculated_height)
        ggsave(file.path(output_dir,paste0("cell_trajectory_color_by_",color_group,".png",collapse = "")), plot = p, dpi=1000,bg="white",height= calculated_height)
        # facet_wrap
        p_facet = plot_cell_trajectory(gbm_cds, color_by = color_group, cell_size = opt$pointsize, show_branch_points = F ) +
              facet_wrap(eval(expr(~!!ensym(color_group))),scales = "free_x", ncol = as.numeric((ifelse(nlevel >20,"4","2")))) +scale_colour_manual( values = user_color_pal) + 
...
ggsave(file.path(output_dir,"groupby_group_pie_plot.pdf"),plot = pie_by_group)
ggsave(file.path(output_dir,"groupby_group_pie_plot.png"),plot = pie_by_group, dpi=1000)

color_use = get_colors(gbm_cds@phenoData@data, "group", palette)
new_celltype_pal = color_use[["new_celltype_pal"]]
new_celltype_pal = na.omit(new_celltype_pal)

pie_by_group_State = gbm.PlotAbundances(gbm_cds, prop.by = "group", group.by = "State", method = "pie", ncol = 4, cols = new_celltype_pal)
ggsave(file.path(output_dir,"groupby_group_State_pie_plot.pdf"),plot = pie_by_group_State)
ggsave(file.path(output_dir,"groupby_group_State_pie_plot.png"),plot = pie_by_group_State, dpi=1000)

pie_by_clusters = gbm.PlotAbundances(gbm_cds, prop.by = "State", group.by = "clusters", method = "pie", ncol = 4,cols = CustomCol2(1:length(table(gbm_cds@phenoData@data[,"State"]))))
ggsave(file.path(output_dir,"groupby_clusters_pie_plot.pdf"),plot = pie_by_clusters)
ggsave(file.path(output_dir,"groupby_clusters_pie_plot.png"),plot = pie_by_clusters, dpi=1000)

color_use = get_colors(gbm_cds@phenoData@data, "clusters", palette)
new_celltype_pal = color_use[["new_celltype_pal"]]
new_celltype_pal = na.omit(new_celltype_pal)

pie_by_clusters_State = gbm.PlotAbundances(gbm_cds, prop.by = "clusters", group.by = "State", method = "pie", ncol = 4, cols = new_celltype_pal)
ggsave(file.path(output_dir,"groupby_clusters_State_pie_plot.pdf"),plot = pie_by_clusters_State)
ggsave(file.path(output_dir,"groupby_clusters_State_pie_plot.png"),plot = pie_by_clusters_State, dpi=1000)
