...
if (col_name == \"clonotype_id\") {
  mainbar_y_label <- \"Intersection clonetype number\"
  sets_x_label <- \"Total clonetype number\"
} else {
  mainbar_y_label <- paste0(\"Intersection \", col_name, \" number\")
  sets_x_label <- paste0(\"Intersection \", col_name, \" number\")
}
pdf(\"$outdir/VennGraph.pdf\", width=14, height=7, onefile=F)
upset(data, nsets=datnum, nintersects=NA, number.angles=30, point.size=2, line.size=1, mainbar.y.label=mainbar_y_label, sets.x.label=sets_x_label, text.scale=c(1.3,1.3,1,1,1.3,1), mb.ratio=c(0.55, 0.45), order.by=\"freq\", show.numbers=\"yes\", sets.bar.color=rainbow(datnum))
dev.off()
