...
option_list = list(
    ...
    make_option( c("--subcluster"),type = "character", default = "all"),
...
if ( is.null(opt$RDS) ){
    ...
    if (opt$subnew_celltype != "all"){
        ...
        seurat_ob@meta.data$new_celltype = as.character(seurat_ob@meta.data$new_celltype)
    ...
    if (opt$subcluster != "all"){
        subcluster = str_split(opt$subcluster,",")[[1]]
        seurat_ob = seurat_ob[,seurat_ob@meta.data$clusters %in% subcluster]
        print(unique(seurat_ob@meta.data$subclusters))
    }
...
for ( vismethod in vismethods ){
    ...
    if ( vismethod == "geneset" ){
        ...
        geneset_R = "/gpfs/oe-scrna/liuchenglong/RaD/scAfterSalesPipline/scAfterSalesPipline/script/M2/addmodule/ggstatsplot.R"
		if ( is.null(opt$pvalue) ){
		system(glue::glue("module purge && source  /home/liuhongyan/miniconda3/bin/activate scvdj  && echo `which Rscript`
