...
option_list = list(
    ...
    make_option( c("--max_cutoff"), type = "character", default = "auto",
               help = "the max cutoff of umap.[default:auto]"),
...
if ( opt$max_cutoff == "auto" ){
    max.cutoff <- gene_range[2]
}else{
    max.cutoff <- as.numeric(opt$max_cutoff)
}
